...
для приложения. Nginx принимает HTTPS-запросы на порту `443`, а запросы на порт
...
```bash
curl -I https://uit14.ru/assets/<имя-файла>
curl -I https://uit14.ru/assets/<имя-файла>
```
...
Проверить настройку после деплоя можно командами:

```bash
curl -I http://uit14.ru
curl -I https://uit14.ru
sudo certbot renew --dry-run --run-deploy-hooks
```

Первый запрос должен вернуть редирект `301`, второй — успешный HTTPS-ответ.
Последняя команда выполняется на сервере и проверяет продление сертификата и
перезагрузку Nginx, не заменяя действующий сертификат.
